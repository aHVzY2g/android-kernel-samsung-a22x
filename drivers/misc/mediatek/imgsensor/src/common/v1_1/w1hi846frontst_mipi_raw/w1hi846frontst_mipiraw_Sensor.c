...
#include "w1hi846frontst_mipiraw_Sensor.h"
...
static kal_uint16 read_n22_hi846_eeprom(kal_uint32 addr)
{
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = W1HI846FRONTST_SENSOR_ID,
	...
	.slim_video_delay_frame = 3,
        ...
        //bug604664 zhouyikuan.wt, modify, 2020/12/23,fix mclk waveform distortion
	.isp_driving_current = ISP_DRIVING_4MA,
	.sensor_interface_type = SENSOR_INTERFACE_TYPE_MIPI,
	.mipi_sensor_type = MIPI_OPHY_NCSI2,
	.mipi_settle_delay_mode = MIPI_SETTLEDELAY_MANUAL, //0,MIPI_SETTLEDELAY_AUTO; 1,MIPI_SETTLEDELAY_MANNUAL
        //bug596482,zhouyikuan.wt,MODIFY,2020/20/22,hi846 front camera preview mirror and flip
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = 
{
 ...
 { 3264, 2448,   252, 504,   2560, 1440,   1280,  720, 0, 0, 1280,  720,   0, 0, 1280, 720 },
};// slim video
...
static void preview_setting(void)
{
	...
	write_cmos_sensor(0x090E, 0x0009);
	write_cmos_sensor(0x0954, 0x0089);
	write_cmos_sensor(0x0956, 0x0000);
	write_cmos_sensor(0x0958, 0xB880);
	write_cmos_sensor(0x095A, 0x6180);
	write_cmos_sensor(0x0F32, 0x025A);
	write_cmos_sensor(0x0F38, 0x0257);
...
static void capture_setting(kal_uint16 currefps)
{
	...
	if (currefps == 300) {
		...
		write_cmos_sensor(0x0958, 0xB880);
#ifdef CONFIG_WINGTECH_S96806
		write_cmos_sensor(0x095A, 0x618e);
		write_cmos_sensor(0x0F32, 0x025A);
		write_cmos_sensor(0x0F38, 0x0256);
#else
		write_cmos_sensor(0x095A, 0x6180);
		write_cmos_sensor(0x0F32, 0x025A);
		write_cmos_sensor(0x0F38, 0x0257);
...
static void hs_video_setting(void)
{
	...
	//Image size      : 640x480
	//MCLK/PCLK       : 24MHz /288Mhz
	//MIPI speed(Mbps): 174Mbps x 4Lane
	//Frame Length    :  631
	//Line Length     : 3800
	...
	write_cmos_sensor(0x2008, 0x38FD);
	write_cmos_sensor(0x326E, 0x0000);
	write_cmos_sensor(0x0710, 0x01F2);
	//========================================================
	//      MIPI 4lane 174Mbps
	//========================================================
	write_cmos_sensor(0x0900, 0x0300);
	...
	write_cmos_sensor(0x0916, 0x0205);
	write_cmos_sensor(0x0918, 0x0202);
	write_cmos_sensor(0x091A, 0x0205);
	write_cmos_sensor(0x091C, 0x0802);
	write_cmos_sensor(0x091E, 0x0400);
	write_cmos_sensor(0x090C, 0x00E6);
	write_cmos_sensor(0x090E, 0x0003);
	write_cmos_sensor(0x0954, 0x0089);
	write_cmos_sensor(0x0956, 0x0000);
	write_cmos_sensor(0x0958, 0xB880);
	write_cmos_sensor(0x095A, 0x6180);
...
static struct stCAM_CAL_DATAINFO_STRUCT w1hi846frontst_eeprom_data ={
	.sensorID= W1HI846FRONTST_SENSOR_ID,
	.deviceID = 0x02,
	.dataLength = 0x076A,
...
static struct stCAM_CAL_CHECKSUM_STRUCT w1hi846frontst_checksum[8] =
{
			...
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				imgsensor.i2c_write_id, *sensor_id);
                                    //+bug604664 zhouyikuan, add, 2020/12/14,hi846 eeprom bring up
                                    size = imgSensorReadEepromData(&w1hi846frontst_eeprom_data,w1hi846frontst_checksum);
                                    if(size != w1hi846frontst_eeprom_data.dataLength || (w1hi846frontst_eeprom_data.sensorVendorid >> 24)!= read_n22_hi846_eeprom(0x0001)) {
                                                pr_err("get eeprom data failed\n");
                                                if(w1hi846frontst_eeprom_data.dataBuffer != NULL) {
                                                        kfree(w1hi846frontst_eeprom_data.dataBuffer);
                                                        w1hi846frontst_eeprom_data.dataBuffer = NULL;
                                                }
                                                *sensor_id = 0xFFFFFFFF;
                                                return ERROR_SENSOR_CONNECT_FAIL;
                                    } else {
                                                pr_debug("get eeprom data success\n");
                                                imgSensorSetEepromData(&w1hi846frontst_eeprom_data);
                                    }
				    //-bug604664 zhouyikuan, add, 2020/12/14,hi846 eeprom bring up
				return ERROR_NONE;
			}
...
static kal_uint32 open(void)
{
	...
	LOG_INF("[open]: PLATFORM:MT6833,MIPI 4LANE\n");
			...
			UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	break;
	//+bug626603 liudijin, add, 2021/03/26,fix ITS:test_sensor_fusion fail
	case SENSOR_FEATURE_GET_OFFSET_TO_START_OF_EXPOSURE:
	    *(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = 90000;
	break;
	//-bug626603 liudijin, add, 2021/03/26,fix ITS:test_sensor_fusion fail
	default:
	break;
...
UINT32 W1HI846FRONTST_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc =  &sensor_func;
	return ERROR_NONE;
}	/*	FH10_HI846_FRONT_LHYX_MIPI_RAW_SensorInit	*/
