...
static inline int imgsensor_check_is_alive(struct IMGSENSOR_SENSOR *psensor)
{
	...
	if (sensorID == 0 || sensorID == 0xFFFFFFFF) {
		...
		err = ERROR_NONE;
		//bug626603 liudijin, add, 2021/04/08,enable w1ov64b40reartxd and w1bf2253deplh3 dual camera mode.
		//+bug604664 chenbocheng.wt, add, 2021/01/13, distinguish depth camera params for dualcam
		if((W1HI4821QREARQT_SENSOR_ID == sensorID) || (W1HI4821QREARSY_SENSOR_ID == sensorID) || (W1OV48B2QREARXL_SENSOR_ID == sensorID) ||(W1OV64B40REARTXD_SENSOR_ID == sensorID)){
			main_sensor_id = sensorID;
		}
		//-bug604664 chenbocheng.wt, add, 2021/01/13, distinguish depth camera params for dualcam
		//+bug604664,zhouyikuan.wt,ADD,2020/12/17,add wide angle info for mmigroup apk
		pr_info("using the S96801 camera module info for mmigroup\n");
		if(psensor_inst->sensor_idx == IMGSENSOR_SENSOR_IDX_MAIN){
			...
			if(!strcmp(psensor_inst->psensor_list->name, "w1hi4821qrearqt_mipi_raw"))
				hardwareinfo_set_prop(HARDWARE_BACK_CAM_MOUDULE_ID, "QIUTAI");
			else if(!strcmp(psensor_inst->psensor_list->name, "w1hi4821qrearsy_mipi_raw"))
				hardwareinfo_set_prop(HARDWARE_BACK_CAM_MOUDULE_ID, "SUNYU");
			else if(!strcmp(psensor_inst->psensor_list->name, "w1ov48b2qrearxl_mipi_raw"))
				hardwareinfo_set_prop(HARDWARE_BACK_CAM_MOUDULE_ID, "XINLI");
			else if(!strcmp(psensor_inst->psensor_list->name, "w1ov64b40reartxd_mipi_raw"))
			...
			if(!strcmp(psensor_inst->psensor_list->name, "w1hi846frontst_mipi_raw"))
				hardwareinfo_set_prop(HARDWARE_FRONT_CAM_MOUDULE_ID, "SHENGTAI");
			else if(!strcmp(psensor_inst->psensor_list->name, "w1gc8034frontdc_mipi_raw"))
				hardwareinfo_set_prop(HARDWARE_FRONT_CAM_MOUDULE_ID, "DONGCI");
			...
			if(!strcmp(psensor_inst->psensor_list->name, "w1c2515depsj_mipi_mono"))
				hardwareinfo_set_prop(HARDWARE_BACK_SUBCAM_MOUDULE_ID, "SHIJIA");
			else if(!strcmp(psensor_inst->psensor_list->name, "w1gc02m1bdepcxt_mipi_mono"))
				hardwareinfo_set_prop(HARDWARE_BACK_SUBCAM_MOUDULE_ID, "CXT");
			else if(!strcmp(psensor_inst->psensor_list->name, "w1gc02m1bdepcxt5_mipi_mono"))
				hardwareinfo_set_prop(HARDWARE_BACK_SUBCAM_MOUDULE_ID, "CXT");
			else if(!strcmp(psensor_inst->psensor_list->name, "w1bf2253deplh_mipi_raw"))
				hardwareinfo_set_prop(HARDWARE_BACK_SUBCAM_MOUDULE_ID, "LHYX");
			else if(!strcmp(psensor_inst->psensor_list->name, "w1bf2253deplh3_mipi_raw"))
				hardwareinfo_set_prop(HARDWARE_BACK_SUBCAM_MOUDULE_ID, "LHYX");
			//-bug604664,zhouyikuan.wt,ADD,2020/12/19,add depth camera info for mmigroup apk
			//+bug604664 liudijin.wt, add, 2021/04/16, distinguish depth camera params for ov64b40 dualcam
			else if(!strcmp(psensor_inst->psensor_list->name, "w1c2515depsj64_mipi_mono"))
				hardwareinfo_set_prop(HARDWARE_BACK_SUBCAM_MOUDULE_ID, "SHIJIA");
			else if(!strcmp(psensor_inst->psensor_list->name, "w1bf2253deplh64_mipi_raw"))
				hardwareinfo_set_prop(HARDWARE_BACK_SUBCAM_MOUDULE_ID, "LHYX");
			//-bug604664 liudijin.wt, add, 2021/04/16, distinguish depth camera params for ov64b40 dualcam
		}else if(psensor_inst->sensor_idx == IMGSENSOR_SENSOR_IDX_MAIN3){
			hardwareinfo_set_prop(HARDWARE_WIDE_ANGLE_CAM, psensor_inst->psensor_list->name);
			if(!strcmp(psensor_inst->psensor_list->name, "w1hi556widexl_mipi_raw"))
				hardwareinfo_set_prop(HARDWARE_WIDE_ANGLE_CAM_MOUDULE_ID, "XINLI");
			else if(!strcmp(psensor_inst->psensor_list->name, "w1gc5035cwidecxt_mipi_raw"))
				hardwareinfo_set_prop(HARDWARE_WIDE_ANGLE_CAM_MOUDULE_ID, "CXT");
			//+bug584789, liudijin.wt, ADD, 2021/01/11, add 3 gong wide camera sc500cs bringup
			else if(!strcmp(psensor_inst->psensor_list->name, "w1sc500cswidely_mipi_raw"))
				hardwareinfo_set_prop(HARDWARE_WIDE_ANGLE_CAM_MOUDULE_ID, "LIANYI");
			//-bug584789, liudijin.wt, ADD, 2021/01/11, add 3 gong wide camera sc500cs bringup
