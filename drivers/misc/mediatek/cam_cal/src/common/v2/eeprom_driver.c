...
//+bug604664, liuxingyu, add, 2020/12/24,hi556 eeprom bring up
#define HI556_MODULE_INFO_SIZE 7
#define HI556_LSC_DATA_SIZE 1868
#define HI556_AWB_DATA_SIZE 16

extern unsigned char hi556_data_info[HI556_MODULE_INFO_SIZE + 1];
...
u32 hi556_vendor_id = 0x06030000;
//+bug604664 liuxingyu, add, 2020/12/24,hi556 eeprom bring up
				...
				unsigned int offset, unsigned int size)
{
	...
	if (plist && plist->readCamCalData) {
		pr_info("i2c addr 0x%x,sensor id=0x%x\n", plist->slaveID,plist->sensorID);
...
int imgSensorSetEepromData(struct stCAM_CAL_DATAINFO_STRUCT* pData){
    ...
    if(pData->deviceID == 0x01){
        ...
        LOG_DBG("zyk set eepromSubData successed\n");
    ...
    //+bug604664 liuxingyu.wt, add, 2020/12/17, wide  eeprom bring up
			   ...
			   size_t size, loff_t *offset)
{
	...
	if (kbuf == NULL)
		return -ENOMEM;
         ...
         if((g_eepromMainData != NULL)&&((W1OV48B2QREARXL_SENSOR_ID == pdata->sensor_info.sensor_id) ||
            (W1HI4821QREARQT_SENSOR_ID == pdata->sensor_info.sensor_id) ||
            (W1HI4821QREARSY_SENSOR_ID == pdata->sensor_info.sensor_id) ||
            (W1OV64B40REARTXD_SENSOR_ID == pdata->sensor_info.sensor_id))){
         ...
         //-bug604664 zhouyikuan, add, 2020/12/18,hi4821q_qt eeprom bring up
         } else if((g_eepromSubData != NULL)&&(W1HI846FRONTST_SENSOR_ID == pdata->sensor_info.sensor_id || W1GC8034FRONTDC_SENSOR_ID == pdata->sensor_info.sensor_id)){
             u32 totalLength = (u32)*offset+ (u32)size;
         ...
         //+bug604664, liuxingyu, add, 2020/12/18, add for hi556&gc5035c eeprom bring up
         } else if((g_eepromWideData != NULL)&&(W1GC5035CWIDECXT_SENSOR_ID == pdata->sensor_info.sensor_id || W1SC500CSWIDELY_SENSOR_ID == pdata->sensor_info.sensor_id )){
             u32 totalLength = (u32)*offset+ (u32)size;

             if((g_eepromWideData->dataBuffer)&&(totalLength <= g_eepromWideData->dataLength)){
                 if(*offset == 1){//check id
                     if(copy_to_user(user_buffer, (u8*)&g_eepromWideData->sensorVendorid, 4)){
                         return -EFAULT;
                     }
                     LOG_DBG("%d,ifCAM_CALIOC_G_READ start! offset=%llu, length=%lu\n",__LINE__,
                         *offset,size);
                 } else {//read otp data
                     if(copy_to_user(user_buffer, g_eepromWideData->dataBuffer+(u32)*offset, size)){
                         return -EFAULT;
                     }
                     LOG_DBG("%d,ifCAM_CALIOC_G_READ start! offset=%llu, length=%lu\n",__LINE__,
                         *offset, size);
                 ...
                 LOG_INF("maybe some error buf(%p)read(%d)have(%d) \n",g_eepromWideData->dataBuffer,totalLength,g_eepromWideData->dataLength);
                 kfree(kbuf);
                 return -EFAULT;
             }
         } else if(pdata->sensor_info.sensor_id == 0x556) {
             if(*offset == 1){
                 copy_to_user(user_buffer,(u8 *)&hi556_vendor_id, 4);
             }else{
                 LOG_INF("%d,hi556,ifCAM_CALIOC_G_READ start! offset=0x%x, length=0x%x, sensorid:0x%x\n",__LINE__,
                     *offset, size, pdata->sensor_info.sensor_id);
                 /*if (size == 0x07 && *offset == 0x40B){//HI556 MID data
                     memcpy(user_buffer, (u8 *)&hi556_module_id, size);
                 }
                 if (size == 0x10 && *offset == 0x41b && hi556_awb_valid){//HI556 AWB data
                     memcpy(user_buffer, (u8 *) &hi556_data_awb[0], size);
                 }
                 if (size == 0x74C && *offset == 0x44f && hi556_lsc_valid){//HI556 LSC data
                     memcpy(user_buffer, (u8 *) &hi556_data_lsc[0], size);
                 }*/
                 copy_to_user(user_buffer+1, (u8 *)&hi556_data_info, HI556_MODULE_INFO_SIZE);
                 copy_to_user(user_buffer+1+HI556_MODULE_INFO_SIZE+2, (u8 *)&hi556_data_awb, HI556_AWB_DATA_SIZE);
                 copy_to_user(user_buffer+1+HI556_MODULE_INFO_SIZE+2+HI556_AWB_DATA_SIZE+2, (u8 *)&hi556_data_lsc, HI556_LSC_DATA_SIZE);
             }
         //-bug604664, liuxingyu, add, 2020/12/18, add for hi556&gc5035c eeprom bring up
         //-bug604664,liuidijin.wt,ADD,2021/01/12,3 gong wide camera sc500cs eeprom bring up
         } else {
