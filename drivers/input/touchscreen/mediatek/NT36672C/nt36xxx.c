...
return:
	Executive outcomes. 0---success. -1---fail.
*******************************************************/
//wanglongfei.wt 20201216 add for tp hardware info
...
info_retry:
    ...
    sprintf(Ctp_name,"TM,NT36672C,FW:0x%02x\n", ts->fw_ver);
...
#ifdef CONFIG_PANEL_NOTIFIER
	ts->fb_notif.notifier_call = nvt_panel_notifier_callback;
	ret = panel_register_client(&ts->fb_notif);
	if(ret) {
...
skip_cmd:
	...
	if (ts->power_status != POWER_OFF_STATUS) {
		...
		//if (device_may_wakeup(&ts->client->dev)) {
			input_info(true, &ts->client->dev, "calling enable_irq_wake\n");
...
static int nvt_panel_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	printk("[NVT_ts]nvt_fb_notifier_callback in *****************\n ");
