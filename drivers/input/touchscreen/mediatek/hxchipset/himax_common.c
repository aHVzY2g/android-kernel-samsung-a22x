...
static struct proc_dir_entry *wt_fac_touch_proc_dir;
static struct proc_dir_entry *wt_fac_proc_self_test_file;
#define WT_FAC_PROC_TOUCH_FOLDER "touchscreen"
#define WT_FAC_PROC_SELF_TEST_FILE "ctp_openshort_test"
...
static void *wt_fac_self_test_seq_start(struct seq_file *s, loff_t *pos)
{
	if (*pos >= 1)
		return NULL;
...
static void *wt_fac_self_test_seq_next(struct seq_file *s, void *v, loff_t *pos)
{
	return NULL;
}

static void wt_fac_self_test_seq_stop(struct seq_file *s, void *v)
{
}

static int wt_fac_self_test_seq_read(struct seq_file *s, void *v)
{
	int ret;

	if (private_ts->suspended == 1) {
		E("%s: please do self test in active mode\n", __func__);
		return HX_INIT_FAIL;
	}
	...
	private_ts->in_self_test = 1;
	himax_int_enable(0);/* disable irq */
	ret = g_core_fp.fp_chip_self_test(NULL, v);
	himax_int_enable(1);
	private_ts->in_self_test = 0;
...
static const struct seq_operations wt_fac_self_test_seq_ops = {
	.start	= wt_fac_self_test_seq_start,
	.next	= wt_fac_self_test_seq_next,
	.stop	= wt_fac_self_test_seq_stop,
	.show	= wt_fac_self_test_seq_read,
};

static int wt_fac_self_test_proc_open(struct inode *inode, struct file *file)
{
	return seq_open(file, &wt_fac_self_test_seq_ops);
}

static ssize_t wt_fac_self_test_write(struct file *filp, const char __user *buff,
			size_t len, loff_t *data)
{
	return len;
}

static const struct file_operations wt_fac_proc_self_test_ops = {
	.owner = THIS_MODULE,
	.open = wt_fac_self_test_proc_open,
	.read = seq_read,
	.write = wt_fac_self_test_write,
	.release = seq_release,
};

static int wt_fac_proc_init(void)
{
	wt_fac_touch_proc_dir = proc_mkdir(WT_FAC_PROC_TOUCH_FOLDER, NULL);
	if (wt_fac_touch_proc_dir == NULL) {
		E(" %s: wt_fac_touch_proc_dir file create failed!\n", __func__);
		return -ENOMEM;
	}

	wt_fac_proc_self_test_file = proc_create(WT_FAC_PROC_SELF_TEST_FILE, 0444,
		wt_fac_touch_proc_dir, &wt_fac_proc_self_test_ops);
	if (wt_fac_proc_self_test_file == NULL) {
		E(" %s: proc wt_fac_proc_self_test_file file create failed!\n", __func__);
		goto fail;
	}
...
fail:
	remove_proc_entry(WT_FAC_PROC_TOUCH_FOLDER, NULL);
