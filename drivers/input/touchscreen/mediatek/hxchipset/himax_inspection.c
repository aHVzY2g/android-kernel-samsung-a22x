...
static size_t get_idx(char *str, char c)
{
	int i;
	char *p = str;
...
static void strcpy_idx(char *str, char start, char end)
{
	...
	start_idx = get_idx(str, start);
	end_idx = get_idx(str, end);
...
static int himax_self_test_data_init(void)
{
	...
	snprintf(g_file_path, sizeof(g_file_path) - 1,
			"%s%s-%d_%d_%d-%d_%d_%d.txt",
			HX_RSLT_OUT_PATH, HX_RSLT_OUT_FILE,
