...
enum lcm_name{
	...
	TRULY_FT8722_MODULE = 2,
	TXD_HX83112F_MODULE = 3,
};
	...
	void *data)
{
	...
	if (event == PANEL_UNPREPARE) {
		input_info(true, &ts->spi->dev, "event=%lu\n", event);
...
static int tp_match_lcm_name(void)
{
	...
	if (strstr(saved_command_line,"nt36672c_fhdp_wt_dsi_vdo_cphy_90hz_tianma")) {
		g_lcm_name = TM_NT36672C_MODULE;
		strcpy((char *)tp_name, "NVT-ts");
	} else if (strstr(saved_command_line,"ili7807s_fhdp_wt_dsi_vdo_cphy_90hz_txd")){
		g_lcm_name = TXD_ILI7807S_MODULE;
	...
	} else if (strstr(saved_command_line,"hx83112f_fhdp_wt_dsi_vdo_cphy_90hz_txd")) {
		g_lcm_name = TXD_HX83112F_MODULE;
