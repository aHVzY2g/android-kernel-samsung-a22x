...
static bool sar_get_boardid(void)
{
	...
	if (br_ptr != 0) {
		...
		if ((!strncmp(board_id, "S96801BA1",strlen("S96801BA1"))) || (!strncmp(board_id, "S96801TA1",strlen("S96801TA1")))
			|| (!strncmp(board_id, "S96802BA1",strlen("S96802BA1")))) {
		     board_is_la = true;
		}
	} else
        board_is_la = false;
		...
		pabov_platform_data_t pplatData)
{
	...
	//+bug676569,jiaoyuxin.wt,modify,20210715,distinguish sar firmware according to Latin America version
	boardid = sar_get_boardid();
	printk(" boardid = %d ", boardid);
	if(boardid)
	{
...
static int abov_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (this) {
        ...
        if (ret < 0) {
            LOG_DBG("Create ch0_cap_diff_dump file failed (%d)\n", ret);
            goto err_class_creat;
        }

		ret = class_create_file(&capsense_class, &class_attr_ch1_cap_diff_dump);
        if (ret < 0) {
            LOG_DBG("Create ch1_cap_diff_dump file failed (%d)\n", ret);
            goto err_class_creat;
        }
        ...
        if (ret < 0) {
            LOG_DBG("Create ch2_cap_diff_dump file failed (%d)\n", ret);
            goto err_class_creat;
        }
#endif
